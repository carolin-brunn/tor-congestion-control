                           ...
                           Ptr<PseudoClientSocket> clientSocket)
{
  ...
  // create PseudoClientSocket if none was passed
  if (!clientSocket)
    {
  ...
  if (!m_disableProxies)
    {
      AddRelay (GetProxyName (id)); // entry relay is also the proxy
    }
...
TorStarHelper::AddRelay (string name)
{
  if (m_relays.find (name) == m_relays.end ())
    {
      // class RelayDecriptor defined in tor-star-helper.h
      // nodeName is added to the tapp pointer
      // tapp = m_factory.Create<TorBaseApp> () (below): 
      //    because of m_factory.SetTypeId ("ns3::TorApp") (above) => TorApp is created
      //    m_factory is of type ObjectFactory (tor-star-helper.h) => Construct a factory for a specific TypeId by name
      RelayDescriptor desc (name, m_nSpokes++, CreateTorApp ());

      // m_relays.type = map<string, RelayDescriptor>
      // add relayDescriptor to relay with corresponding name
      m_relays[name] = desc;

      // m_relayApps.type = ApplicationContainer => ns3 class: holds a vector of ns3::Application pointers
      // add TorApp of relayDesciptor to the corresponding relayApps
...
TorStarHelper::RegisterTtlbCallback (void (*ttlb)(int, double, string), string hint)
{
  ...
  for (i = m_circuits.begin (); i != m_circuits.end (); ++i)
    {
      CircuitDescriptor desc = i->second; // i is map => first: int; second: CircuitDescriptor
...
{
  m_starHelper = new PointToPointStarHelper (m_nSpokes,m_p2pHelper); // args: number of links attached to the hub node, helper to link nodes together

  // Configure specific link delays
  for (auto it = m_specificLinkDelays.cbegin(); it != m_specificLinkDelays.cend(); ++it) // m_specificLinkDelays = map<string, Time>
  {
...
TorStarHelper::InstallCircuits ()
{
  ...
  for (i = m_circuits.begin (); i != m_circuits.end (); ++i)
    {
      CircuitDescriptor e = i->second; // i = map => second = CircuitDescriptor

      // add desc.tapp for all relays
      // remembers app and schedules with context
      if (!m_disableProxies)
        {
      ...
      // TODO: WHY are the cnnections solved like this with previous and next?! -> bidrectionality?!
      // ??? server -> exit -> middle => p_ip = server (previous), n_ip = n_ip (next) 
      exitApp->AddCircuit (e.id, serverAddress, SERVEREDGE, middleAddress, RELAYEDGE); // add circuit between server and middle
      // ??? exit -> middle -> entry
      middleApp->AddCircuit (e.id, exitAddress, RELAYEDGE, entryAddress, RELAYEDGE); // add circuit between exit and entry
      if (!m_disableProxies)
        {
          // ??? middle -> entry -> client (= proxy)
  ...
  if (m_starHelper->GetSpokeNode (desc.spokeId)->GetNApplications () == 0 )
    {
      ...
      m_starHelper->GetSpokeNode (desc.spokeId)->AddApplication (desc.tapp); // hinterlegt App und schedule fpr simulation => will be used when Simulation::RUN() is called
      // in ns3/node.cc Simulator::ScheduleWithContext (GetId (), Seconds (0.0), &Application::Initialize, application)
      // the scehdule then calls Application::Initialize: calls the virtual DoInitialize() method on all the Objects aggregated to this Object; will be called only once!
    }
