...
def load_data_new(path, merge_dict, n_circ, run, cong, datarate_dict, relay):
    ...
    for c in range(1,n_circ+1):
        ...
        df_circ = df_circ[df_circ[0].str.contains("RECEIVED")] 
        df_circ = df_circ.reset_index(drop = True)

        l_recv_time = df_circ.apply(lambda row: process_row(row, 0), axis=1)
        l_delwnd = df_circ.apply(lambda row: process_row(row, 3), axis=1)
        l_recv_pckcnt = df_circ.apply(lambda row: process_row(row, 4), axis=1)
        
        merge_dict["time"].extend(l_recv_time)
        merge_dict["delwnd"].extend(l_delwnd)
        merge_dict["packet_cnt"].extend(l_recv_pckcnt)
        merge_dict["circuit"].extend(([str(c)]*len(l_recv_time)))
        merge_dict["run"].extend([str(run)]*len(l_recv_time))
        merge_dict["cong_flavor"].extend([cong] * len(l_recv_time))
        
        final_cnt = l_recv_pckcnt[len(l_recv_pckcnt)-1]
        first_time = l_recv_time[0]
        last_time = l_recv_time[len(l_recv_time)-1]
...
def main():
    ...
    dict_name = "tor_simu_recvPckCnt_BDP" + bdpFlavor + "_nCirc" + str(n_circ) + "_simt" + sim_t + "_rtt" + str(rtt) + ".csv"
    merged_df_old.to_csv(dict_name, sep = ',')
    
    datarate_df = pd.DataFrame(datarate_dict)
    dict_name = "tor_simu_recvFinalCnt_BDP" + bdpFlavor + "_nCirc" + str(n_circ) + "_simt" + sim_t + "_rtt" + str(rtt) + ".csv"
    ...
    plt.savefig("plt_CDF_recv_pckcnt_bdp" + bdpFlavor + "_nCirc" + str(n_circ) + "_simt" + sim_t + "_rate" + bwRate + "_burst" + bwBurst+ "_run" + str(run) + "_rtt" + str(rtt) +".pdf")
    plt.close()
    ...
    plt.savefig("plt_CDF_recv_finalcnt_bdp" + bdpFlavor + "_nCirc" + str(n_circ) + "_simt" + sim_t + "_rate" + bwRate + "_burst" + bwBurst+ "_run" + str(run) + "_rtt" + str(rtt)+".pdf")
    plt.close()
    ...
    plt.savefig("plt_CDF_recv_datarate_bdp" + bdpFlavor + "_nCirc" + str(n_circ) + "_simt" + sim_t + "_rate" + bwRate + "_burst" + bwBurst+ "_run" + str(run) + "_rtt" + str(rtt)+".pdf")
    plt.close()
